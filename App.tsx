...
import { INITIAL_EVENTS, INITIAL_TICKETS } from './constants';
...
import { Ticket as TicketIcon, Moon, Sparkles, LogOut, ShieldCheck, Map, Database, AlertTriangle, User as UserIcon } from 'lucide-react';
  ...
  const handleBuyTicket = (event: Event) => {
    ...
    if (targetEvent.availableTickets > 0) {
      const newTicket: Ticket = {
        ...
        ticketCode: `LUNAR-${Math.random().toString(36).substr(2, 9).toUpperCase()}`,
        qrCode: `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=LUNAR-${targetEvent.id}-${user.id}`
      };
