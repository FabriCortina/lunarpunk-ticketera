...
export interface TicketWithEvent extends TicketEntity {
  ...
  organizer_id: string; // Needed for validation ownership check
}
...
export class TicketRepository {
  ...
  // Actualización atómica para evitar race conditions en validación
  async markAsValidated(id: string): Promise<TicketEntity | undefined> {
    const [ticket] = await this.db('tickets')
      .where({ id, status: 'PAID' }) // Solo actualiza si está PAID
      .update({
        status: 'VALIDATED',
